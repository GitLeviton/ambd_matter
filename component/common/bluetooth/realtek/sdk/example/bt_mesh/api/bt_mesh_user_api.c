...
static const mesh_cmd_entry mesh_cmd_table[] = {
    ...
    PARAM_MESH_CODE("gbg", "\r gbg [dst] [app_key_index]", "\r generic battery get\n\r", GEN_MESH_CODE(_generic_battery_get))   // LEV-MOD
    PARAM_MESH_CODE("gbs", "\r gbs [dst] [app_key_index]", "\r generic battery set\n\r", GEN_MESH_CODE(_generic_battery_set))   // LEV-MOD
...
extern int Lev_bt_mesh_send_io_msg(T_IO_MSG *p_io_msg); // LEV-MOD
...
int bt_mesh_user_cmd(uint16_t mesh_code, void *pmesh_cmd_item_s)
{
	...
	ret = Lev_bt_mesh_send_io_msg(&io_msg); // LEV-MOD
...
user_api_parse_result_t bt_mesh_set_user_cmd(uint16_t mesh_code, user_cmd_parse_value_t *pparse_value, user_cmd_cbk cbk, void *user_data)
{
    ...
    if (mesh_code == GEN_MESH_CODE(_prov)) {
        time_out = 20000;       //LEV-MOD
    } else {
        time_out = 12000;        //LEV-MOD
    }
#else
    time_out = 12000;        //LEV-MOD
#endif
...
user_api_parse_result_t bt_mesh_indication(uint16_t mesh_code, uint8_t state, void *pparam)
{
    ...
    printf("\r\n INDICATION %s()",__func__); // LEV-MOD
    Lev_BT_Mesh_Indication(mesh_code, state, NULL); // LEV-MOD
#endif
