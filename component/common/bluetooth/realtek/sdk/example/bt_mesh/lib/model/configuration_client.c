...
#include "Lev_Project_Config.h"	// LEV-MOD
#include "Lev_Bluetooth_Mesh.h" //LEV-MOD
...
// LEV-MOD
#if (1) //defined(CONFIG_BT_MESH_PROVISIONER_RTK_DEMO) && CONFIG_BT_MESH_PROVISIONER_RTK_DEMO
#include "Lev_bt_mesh_app_lib_intf.h"       // LEV-MOD
extern struct BT_MESH_LIB_PRIV bt_mesh_lib_priv;
extern void update_node_group(uint16_t mesh_addr, uint16_t group_addr);
#include "Lev_vendor_model.h" //LEV-MOD
#endif
...
bool cfg_client_receive(mesh_msg_p pmesh_msg)
{
    ...
    if((pmesh_msg->access_opcode & 0xF0FFFF ) == MESH_MSG_LEV_VENDOR_MODEL_OPCODE) {  //LEV-MOD
        /*
        uint8_t opcode = (pmesh_msg->access_opcode & 0x0F0000) >> 16; //LEV-MOD

        printf("\nRYAN CLIENT RECEIVE 0x%x, 0 - 0x%02x, 1 - 0x%02x, 2 - 0x%02x, 3 - 0x%02x, 4 - 0x%02x, 5 - 0x%02x, 6 - 0x%02x, 7 - 0x%02x, 8 - 0x%02x\n", pmesh_msg->access_opcode,
               pbuffer [0], pbuffer [1], pbuffer [2], pbuffer [3], pbuffer [4], pbuffer [5], pbuffer [6], pbuffer [7], pbuffer [8]);

        MESSAGE_ENTRY_T msg; //LEV-MOD
        msg.opcode = opcode; //LEV-MOD
        msg.attr = (pbuffer [3] << 8); //LEV-MOD
        msg.attr |= pbuffer [4]; //LEV-MOD
        msg.trid = pbuffer [6]; //LEV-MOD
        __global_trans_id = msg.trid;
        msg.data_len = pbuffer [7]; //LEV-MOD
        memcpy(msg.data, &pbuffer[8], msg.data_len); //LEV-MOD

        //handle_device_message_receive(pmesh_msg->pmodel_info, pmesh_msg->src, pmesh_msg->app_key_index, &msg); //LEV-MOD
        handle_device_message_receive(&lev_vendor_server_model, pmesh_msg->src, pmesh_msg->app_key_index, &msg); //LEV-MOD
*/
        return false;
    } //LEV-MOD
    else { //LEV-MOD
		printf ("\nRYAN CLIENT RECEIVE 0x%x  free heap size: [%d]\n", pmesh_msg->access_opcode, xPortGetFreeHeapSize());
...
#ifdef BLUETOOTH_MESH_ENABLED				
                Lev_BT_Mesh_Indication(MESH_MSG_CFG_NET_TRANS_SET,BT_MESH_USER_CMD_SUCCESS, NULL);    // LEV-MOD
#endif
            }
            break;
        case MESH_MSG_CFG_MODEL_PUB_STAT:
            {
                ...
                {
                    parse = TRUE; // LEV-MOD
                    data_uart_debug("\nMESH_MSG_CFG_MODEL_PUB_STAT Success!\n");
                    data_uart_debug("Pub stat: pub addr = 0x%04x ttl = 0x%04x!\n", pmsg->pub_addr, pmsg->pub_ttl);
                    data_uart_debug("Pub stat: elem addr = 0x%04x app_key_index = %d!\n", pmsg->element_addr, pmsg->pub_key_info.app_key_index);
                    data_uart_debug("Pub stat: frnd flag = 0x%04x pub_retrans_info.steps = 0x%04x!\n", pmsg->pub_key_info.frnd_flag, pmsg->pub_retrans_info.steps);
                    data_uart_debug("Pub stat: pub_period = 0x%04x pub_retrans_info.count = 0x%04x!\n", pmsg->pub_period, pmsg->pub_retrans_info.count);
    #if defined(CONFIG_EXAMPLE_BT_MESH_DEMO) && CONFIG_EXAMPLE_BT_MESH_DEMO
